...
if (isset($_GET['delete'])) {
    ...
    $checksql = "SELECT * FROM internship WHERE assessor_id = '$aid'";
    $checkresult = $conn->query($checksql);

    if ($checkresult->num_rows > 0) {
        $msg = "Cannot delete this assessor because this assessor already has internship record.";
        $msgtype = "error";
    } 
    else {
        ...
        if ($row) {
            ...
            $deleteassessor = "DELETE FROM assessor WHERE assessor_id = '$aid'";
            $deleteuser = "DELETE FROM users WHERE user_id = '$uid'";

            if ($conn->query($deleteassessor) && $conn->query($deleteuser)) {
                $msg = "Assessor deleted successfully.";
                $msgtype = "success";
            } 
            else {
